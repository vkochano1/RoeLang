...
namespace roe
{
  ...
  public:
    ...
    void registerExternal(const std::string& name, FPTR fptr, llvm::Type* ret,
    ...
    void registerExternal_(const std::string& name, void* fptr, llvm::Type* ret,
                           std::initializer_list<llvm::Type*> params);
    ...
    llvm::CallInst* makeCall(const std::string&                  name,
                             std::initializer_list<llvm::Value*> arg_list);

    void applyGlobalMapping(ExecutionEngine&   executionEngine,
