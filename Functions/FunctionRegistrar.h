...
namespace roe
{
  ...
  public:
    ...
    void registerExternal(
      const std::string& name, FPTR fptr, llvm::Type* ret, std::initializer_list<llvm::Type*> params)
    {
      registerExternal_(name, reinterpret_cast<void*>(fptr), ret, params);
    }

    template<typename FPTR>
    void registerExternal(const std::string& name, FPTR fptr, llvm::Type* ret, const Parameters& params)
    {
      registerExternal_(name, reinterpret_cast<void*>(fptr), ret, params);
    }

    void registerExternal_(
      const std::string& name, void* fptr, llvm::Type* ret, std::initializer_list<llvm::Type*> params);

    void registerExternal_(const std::string& name, void* fptr, llvm::Type* ret, const Parameters& params);
