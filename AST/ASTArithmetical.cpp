   ...
   ASTArithmetical::ASTArithmetical(Context& context, Operator op, ASTElementPtr op1, ASTElementPtr op2)
                                            ...
                                            , llvm::Value*& out)
    {
            ...
            && right->getType() == context_.types().stringPtrType())
        {
            ...
            context_.externalFunctions().makeCall(StringOps::CONCAT_STR_AND_STR, { left, left, right} );
        }
        ...
        {
            out = builder.CreateAlloca(context_.types().stringType());
            context_.externalFunctions().makeCall(StringOps::CONCAT_STR_AND_CHPTR, { left, right, out} );
        }
        ...
        {
            out = builder.CreateAlloca(context_.types().stringType());
            context_.externalFunctions().makeCall(StringOps::CONCAT_CHPTR_AND_STR, { left, right, out} );
        }
        else if (left->getType() == context_.types().charPtrType()
                 && right->getType() == context_.types().charPtrType()
                )
        {
            out = builder.CreateAlloca(context_.types().stringType());
            context_.externalFunctions().makeCall(StringOps::CONCAT_CHPTR_AND_CHPTR, { left, right, out} );
        }
